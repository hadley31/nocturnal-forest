...
public class Item : MonoBehaviour
{
	...
	public Dictionary<string, int> GetStatBoosts()
	{
		Dictionary<string, int> dict = new Dictionary<string, int>();
		foreach (var boost in GetComponents<MixinItemStatBoost>())
		{
			if (dict.ContainsKey(boost.Stat))
			{
				dict[boost.Stat] += boost.Amount;
			}
			else
			{
				dict.Add(boost.Stat, boost.Amount);
			}
		}
		return dict;
	}
