...
public class Inventory : MonoBehaviour
{
	...
	// This is the most inefficient code I have ever written but it will work fine for now
    public bool GetAbilityOverride(string ability)
    {
		MixinItemAbilityOverride max = null;

        foreach (Item item in items)
        {
            foreach (var ao in item.GetComponents<MixinItemAbilityOverride>().Where(x => x.Ability == ability))
            {
				if (max == null || ao.Precedence > max.Precedence){
					max = ao;
				}
            }
        }

		return max?.Override ?? false;
    }
}
