...
public class EntityHurt : TriggerBase
{
	public int damage = 0;
	public float hurtInterval = 1;

	private float m_NextHurtTime = 0;
	...
	private void Update ()
	{
		...
		if ( Time.time > m_NextHurtTime && m_HurtList.Count > 0 )
		{
			Hurt ();
		}
	}
	...
	private void Hurt ()
	{
		...
		m_NextHurtTime = Time.time + hurtInterval;
	}
	...
	{
		m_HurtList?.RemoveAll (x => x == null || x.gameObject.activeSelf == false || x.GetComponent<Collider2D> ().enabled == false || x.GetComponent<Collider2D> ().IsTouching (Trigger.Collider) == false);
	}
}
